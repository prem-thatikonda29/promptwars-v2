...
export function SosButton() {
  ...
  return (
    <>
      ...
      {showConfirm && (
        <div className="fixed inset-0 bg-black/40 z-[60] flex items-center justify-center px-4 animate-fade-in">
          <div className="bg-white rounded-xl border border-[#DADCE0] shadow-2xl p-5 max-w-sm w-full flex flex-col gap-4">
            <div className="flex items-start justify-between">
              <div className="flex items-center gap-2.5">
                <div className="p-2 bg-[#FEF7E0] text-[#F9AB00] rounded-lg">
                  <AlertCircle className="w-5 h-5" />
                </div>
                <div>
                  <h3 className="text-sm font-bold text-[#202124] tracking-tight">Active Alert Detected</h3>
                  <p className="text-[11px] text-[#5F6368]">
                    You triggered an SOS {cooldownRemaining}s ago (#{sosCountInWindow} in this window)
                  </p>
                </div>
              </div>
              <button onClick={handleConfirmNo} className="p-1 text-[#80868B] hover:text-[#202124] cursor-pointer">
                <X className="w-4 h-4" />
              </button>
            </div>
      ...
      <div className="hidden md:flex bg-white border border-[#DADCE0] rounded-xl p-4 items-center justify-between gap-4 shadow-xs">
        <div className="flex items-center gap-3 text-left">
          <div className="p-2.5 bg-[#FCE8E6] text-[#D93025] rounded-lg shrink-0">
        ...
        {status === "sent" ? (
          <div className="flex items-center gap-3 shrink-0">
            ...
            {cooldownRemaining > 0 && (
              <span className="text-[10px] font-mono text-[#80868B]">{cooldownRemaining}s</span>
            )}
            <button
      ...
      <div className="md:hidden fixed bottom-5 left-1/2 -translate-x-1/2 z-50 w-[92%] max-w-sm">
        {status === "sent" ? (
          <div className="bg-[#188038] text-white p-3 px-4 rounded-full shadow-xl border border-white/20 flex items-center justify-between gap-3 animate-fade-in">
          ...
          >
            {status === "locating" || status === "sending" ? (
              ...
              <>
                <AlertCircle className="w-5 h-5 fill-white text-[#D93025] animate-pulse-live" />
